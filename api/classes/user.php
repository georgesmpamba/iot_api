...
class Utilisateur{
    ...
    function __construct(){
     ...
     public  function show(int $id=0,bool $all=true){
            ...
            $db_con= $this->str_db;
            $sql= $db_con->prepare($all ? "SELECT * FROM users WHERE :id" : "SELECT * FROM users WHERE id_user =:id" );
            $car=[];
                
            try {
                $sql->execute([':id'=>$all ? 1: $id]);//Exécute la requête dynamiquement
                while($user=$sql->fetch(PDO::FETCH_ASSOC)){
                    $user['id_user']=(int) $user['id_user'];
                    if(isset( $user['mdp'])){
                        unset($user['mdp']);
                    }
                    $car[]=$user;
    ...
    /*Méthode pour enregistrer un Utilisateur */
        public function save(string $nom="",string $post_nom="",string $prenom="",string $sexe="",string $num_tel="",string $adresse="",string $mdp="1234",string $temp=""){
            ...
            $contact=["email"=>"","phone"=>$num_tel];//contact de l'utilisateur
            $sql=$db_con->prepare("INSERT INTO users(nom_user,postnom_user,prenom_user,sexe_user,contact_user,adresse_user,mdp,date_user,temp) VALUES (:nom,:post_nom,:prenom,:sexe,:contact,:adresse,:mdp,:temp)");
            try {
                if($sql->execute([':nom'=>$nom,':post_nom'=>$post_nom,':prenom'=>$prenom,':sexe'=>$sexe,':contact'=>$contact,':adresse'=>$adresse,':mdp'=>password_hash ($mdp,PASSWORD_DEFAULT),':temp'=>$temp])){
    ...
    /*Les communtateurs des données  */
        public function delete(int $id=0){
            /*Méthode pour supprimer un Utilisateur*/
                $db_con=$this->str_db;//Récupère la connexion
                $sql=$db_con->prepare("DELETE FROM users WHERE id_user=:id");
            try {
