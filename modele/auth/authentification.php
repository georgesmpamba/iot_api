...
require_once 'token.php';//Intègre le fichier de TOKEN

$tokenObj=new Token();//Objet pour le Token
...
class Auth{
        ...
        if($open->rowCount()>0){//Si les informations sont trouvées
            if(password_verify($pass,$user['mdp'])){//si les informations de connexion sont valides
                ...
                //Enregistre le Token dans la Base des données
                 $tokenObj->addToken($this->token,(int) $user['id'],12);
                 return true;
            }
        }
        return false;
    ...
    public function verifyToken(string $token=''){
        ...
        $got=$tokenObj->verifyToken($token);//récupère les champs
